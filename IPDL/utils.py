...
def show_ip(ip_df: pd.DataFrame, n=25, labels=[]):
    '''
    ...
    with plt.style.context('seaborn'):
        ...
        for idx, (layer, _) in enumerate(ip_df.columns[::2]):
            Ity = moving_average(ip_df[layer]['Ity'].to_numpy(), n=15)
            Ixt = moving_average(ip_df[layer]['Ixt'].to_numpy(), n=15)
            
            cmap = plt.cm.get_cmap(colors[idx])
            iterations = np.geomspace(1, len(Ity)-1, num=n, dtype=np.uint)

            color = np.array([cmap(iterations[int(len(iterations)*0.65)])])
...
def show_aeip(ip_df: pd.DataFrame, ref_entropy: float, n=25):
    '''
    ...
    labels_encoder = list(map(lambda x: 'Encoder {}'.format(x) if x != btnck_idx else 'Bottleneck', encoder_idx))
    labels_decoder = list(map(lambda x: 'Decoder {}'.format(x) if x != btnck_idx else 'Bottleneck', decoder_idx))
    ...
    with plt.style.context('seaborn'):
        ...
        for idx, (layer, _) in enumerate(ip_df.columns[::2]):
            ...
            if idx < btnck_idx-1:
                ...
                if idx == btnck_idx-1:
                    f8_ax1.scatter(Ixt[iterations], Ity[iterations], c=iterations, vmin=0, vmax=iterations.max(), label=labels[idx], marker=markers[idx], cmap=cmap, edgecolors='black')            
                axis = f8_ax2

            axis.scatter(Ixt[iterations], Ity[iterations], c=iterations, vmin=0, vmax=iterations.max(), label=labels[idx], marker=markers[idx], cmap=cmap, edgecolors='black')
        ...
        norm = mpl.colors.Normalize(vmin=0, vmax=len(Ixt))
